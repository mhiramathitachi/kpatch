...
static void kpatch_dump_monitoring_tasks(void)
{
	...
	spin_lock(&kpatch_montask_lock);
	list_for_each_entry(mt, &kpatch_montasks, list) {
		pr_info("PID:%d CHK:%c FAIL:%c\n", mt->pid,
			mt->checked?'y':'n', mt->failed?'y':'n');
	}
	spin_unlock(&kpatch_montask_lock);
}
...
static int kpatch_montask_handler(struct kprobe *kp, struct pt_regs *regs)
{
	...
	if (current_kpatch_module) {
		...
		if (!mt || !mt->checked) {
			dump_trace(current, NULL, NULL, 0,
				   &kpatch_backtrace_ops, &args);
			ret = kpatch_add_monitoring_task(current, true,
							 !!args.ret);
			if (ret < 0) {
...
struct kprobe kpatch_montask_probe = {
...
static int kpatch_verify_activeness_safety(struct kpatch_module *kpmod)
{
	...
	smp_rmb();
	/* Hack the task switching */
	ret = register_kprobe(&kpatch_montask_probe);
	if (ret < 0) {
		pr_info("Error to register task switch hack: %d\n", ret);
		return ret;
	}
...
unlock:
	...
	timeout = 128;
	/* Wait for the rest processes */
	while (kpatch_unchecked_monitoring_tasks(&failed) && failed == 0) {
		synchronize_sched();
		if (--timeout == 0) {
			pr_info("Timeout safeness check\n");
...
out:
	...
	unregister_kprobe(&kpatch_montask_probe);
	kpatch_montask_probe.addr = 0;	/* Cleanup */
